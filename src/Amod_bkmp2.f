!=================================================================================
!     1D potential for H-H vibration
!     r: H-H bond length
...
!=================================================================================
      module mod_mkmp2
         private
         public:: refpot,potface1
         contains
      subroutine refpot(r,v)
      implicit   none
      real*8     r,v(1,1),x(3)
...
!=================================================================================
      subroutine potfacem(NPES,kcoord,AM,rs,r1,a1,vx)
      implicit none
      integer,parameter :: nstates=1,natm=3
      real*8,parameter :: rcut=0.1d0,vcut=3.d0/27.2114d0
      integer  NPES, kcoord
      real*8   AM(3),rs,r1,a1,vx(NPES,NPES)
      real*8   x(3,natm),r(3)
      real*8   e(nstates)
      real*8   hd(nstates,nstates),ckl(nstates,nstates)
      real*8   cg(3*natm,nstates,nstates)
      real*8   dcg(3*natm,nstates,nstates)

      if(NPES.ne.nstates) stop !'Error: wrong NPES value! the current PES of BeH2+ system has 3 states'
      call jacob2cart(kcoord,AM,rs,r1,a1,x)
      call cart2bond(r,x)
      if(r(1).le.rcut.or.
     &   r(2).le.rcut.or.
     &   r(3).le.rcut) then
...
!=================================================================================
      subroutine potface1(IPES,kcoord,AM,rs,r1,a1,vx)
         !RS: R
         !r1: r
         !a1: theta
      implicit none
      ...
      real*8,parameter :: rcut=0.1d0,vcut=3.d0/27.2114d0
      integer  IPES, kcoord
      real*8   AM(3),rs,r1,a1,vx
      real*8   x(3,natm),r(3)
      real*8   e(nstates)
      real*8   hd(nstates,nstates)

      if(IPES.le.0) stop 'Error: invalid IPES value'
      if(IPES.gt.nstates) stop !'Error: IPES is larger than NPES! the current PES of BeH2+ system has 3 states'
      call jacob2cart(kcoord,AM,rs,r1,a1,x)
      call cart2bond(r,x)
...
!=================================================================================
      subroutine jacob2cart(kcoord,AMASS,Z,r,theta,x)
        implicit none
        integer  kcoord
        real*8   fac
        real*8, intent(in) :: AMASS(3)
        real*8, intent(in) :: Z,r,theta
        real*8, intent(out) :: x(3,3)
        ...
        case(1) ! A + BC
           x(1,1)=Z*cos(theta)
           x(2,1)=Z*sin(theta)
           fac=AMASS(3)/(AMASS(2)+AMASS(3))
           x(1,2)=     -fac *r
           x(1,3)=(1.d0-fac)*r
        case(2) ! B + CA
           x(1,2)=Z*cos(theta)
           x(2,2)=Z*sin(theta)
           fac=AMASS(1)/(AMASS(1)+AMASS(3))
           x(1,3)=     -fac *r
           x(1,1)=(1.d0-fac)*r
        case(3) ! C + AB
           x(1,3)=Z*cos(theta)
           x(2,3)=Z*sin(theta)
           fac=AMASS(2)/(AMASS(1)+AMASS(2))
           x(1,1)=     -fac *r
           x(1,2)=(1.d0-fac)*r
        case default
           stop 'undefined value of kcoord in jacob2cart'
        end select
        return
      end
      !=================================================================================
      subroutine cart2jacob(kcoord,AMASS,x,Z,r,theta)
        implicit none
        integer  kcoord
        real*8   fac
        real*8, intent(in) :: AMASS(3)
        real*8, intent(in) :: x(3,3)
        real*8, intent(out) :: Z,r,theta
        real*8 :: dx(3), dx1(3)
        ...
        case(1) ! A + BC
           ...
           dx1=(x(:,2)*AMASS(2)+x(:,3)*AMASS(3))/(AMASS(2)+AMASS(3))
           dx1=x(:,1)-dx1
           Z=sqrt(dot_product(dx1,dx1))

           theta=dot_product(dx,dx1)/(r*Z)
           theta=dacos(theta)
        case(2) ! B + CA
           dx=x(:,1)-x(:,3)
           r=sqrt(dot_product(dx,dx))

           dx1=(x(:,1)*AMASS(1)+x(:,3)*AMASS(3))/(AMASS(1)+AMASS(3))
           dx1=x(:,2)-dx1
           Z=sqrt(dot_product(dx1,dx1))
        ...
        case(3) ! C + AB
           ...
           dx1=(x(:,1)*AMASS(1)+x(:,2)*AMASS(2))/(AMASS(1)+AMASS(2))
           dx1=x(:,3)-dx1
           Z=sqrt(dot_product(dx1,dx1))
...
!=================================================================================
      ...
      r=0.d0
      do i=1,3
        r(1)=r(1)+(x(i,2)-x(i,3))**2
        r(2)=r(2)+(x(i,3)-x(i,1))**2
        r(3)=r(3)+(x(i,1)-x(i,2))**2
      enddo
...
c  _/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/
      ...
      dimension  x(3),vev(6),vd1(3)

      if(npes.eq.1)then  ! stop 'npes=/3 in pot0 !'
        call bkmp2(x,vev1,vd1,-1)
 	  vev1=vev1+(4.47809D0+0.27018326D0)/27.2114d0
        vdia(1,1)=vev1
	else
	  stop 'NPES WRONG IN POT0!'
	endif
